...
export default function SalesPage() {
  ...
  // [NEW] Master Date Allocation
  const [editingDateSale, setEditingDateSale] = useState<Sale | null>(null);
  ...
  const handleQuickRegister = async (nameToRegister: string) => {
    ...
    try {
      ...
      if (response.ok && data.client) {
        success(`Cliente '${data.client.name}' cadastrado!`);
        ...
        // Clear Search
        const input = document.getElementById('client-search-input') as HTMLInputElement;
  ...
  const handleUpdateDate = async () => {
    ...
    try {
       const res = await fetch(`/api/sales/${editingDateSale.id}/date`, {
          method: "PUT",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${localStorage.getItem("token")}` },
          body: JSON.stringify({ saleDate: newSaleDate })
       });
  ...
  return (
    <div className="p-8 space-y-6 text-white">
                    ...
                    <div className="flex gap-2 md:ml-2">
                      ...
                      {isAdmin && (
                         <div className="flex items-center gap-1">
                          ...
                          {/* [NEW] Edit Date Button for Admin */}
                          <Button 
                             size="sm"
                             variant="ghost"
                             className="rounded-full px-2 py-1 text-gray-500 hover:bg-blue-500/10 hover:text-blue-400"
                             title="Editar Data & Recalcular"
                             onClick={() => {
                                setEditingDateSale(sale);
                                try {
                                    const d = new Date(sale.saleDate);
                                    if(!isNaN(d.getTime())) {
                                        setNewSaleDate(d.toISOString().split('T')[0]);
                                    }
                                } catch {}
                             }}
                          >
              ...
              <div className="relative">
                 <input
                     ...
                     onChange={(e) => {
                        ...
                        // Reset selection if user changes text manually
                        // setFormData(prev => ({ ...prev, clientId: "" })); // Option: Force re-selection
                     }}
                     onFocus={() => setShowClientList(true)}
                     onBlur={() => setTimeout(() => setShowClientList(false), 200)} // Delay to allow click
                 />
      ...
      {/* [NEW] Edit Date Modal */}
      <Modal
        open={Boolean(editingDateSale)}
        onClose={() => setEditingDateSale(null)}
        title="Alterar Data da Venda (Master)"
      >
        <div className="space-y-4">
             ...
             <div className="space-y-2">
                <label className="text-gray-400 text-sm uppercase">Nova Data (para corrigir comissão)</label>
                <input 
                   type="date"
                   value={newSaleDate}
             ...
             <div className="flex justify-end gap-2 pt-2">
                <Button
                  variant="secondary"
                  onClick={() => setEditingDateSale(null)}
                ...
                <Button
                  ...
                  disabled={updatingDate || !newSaleDate}
                  className="bg-blue-600 hover:bg-blue-700"
                >
                  {updatingDate ? "Atualizando..." : "Salvar Nova Data"}
                </Button>
             </div>
        </div>
      </Modal>
