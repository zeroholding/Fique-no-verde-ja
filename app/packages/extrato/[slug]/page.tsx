...
function PackagesStatementContent() {
  ...
  const hasValidSlug = useMemo(() => Boolean(slug), [slug]);
  ...
  const fetchStatement = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/packages/public-statement-by-slug?${urlParams.toString()}`);
    ...
    <div className="min-h-screen bg-gradient-to-b from-black via-[#0f1115] to-black text-white">
      <div className="max-w-5xl mx-auto px-4 py-10 space-y-6">
        <div className="rounded-3xl bg-white/5 border border-white/10 shadow-2xl p-6 space-y-2">
          <p className="text-xs uppercase tracking-[0.3em] text-blue-300/70">Extrato oficial</p>
          ...
          <div className="flex items-center gap-3">
            <Button size="sm" className="rounded-xl" onClick={fetchStatement} disabled={loading || !hasValidSlug}>
...
export default function PublicPackagesStatementBySlugPage() {
  return (
    <Suspense
      fallback={
